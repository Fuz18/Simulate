...
command1=r'pyuic5 -o 车行为界面.py 车行为界面.ui'
os.system(command1)
...
os.chdir(sys.path[0])  # 更改当前工作目录到脚本所在的目录
import 车行为界面  as car_ui 
# import EV_simulate as EV
...
class MyWindow(QMainWindow,car_ui.Ui_Form):
    variables=''
    def __init__(self,parent=None):
        super(MyWindow,self).__init__(parent)
        self.ui = car_ui.Ui_Form()
        self.ui.setupUi(self)# 这句话的意思是将zhuanlun文件放到新建的窗口中
        self.init_ui()
        self.auto_resize()
        QtCore.QCoreApplication.setAttribute(QtCore.Qt.AA_EnableHighDpiScaling)  # 1、适应高DPI设备
    def init_ui(self):   
        ui=self.ui
        ui.push_calculate.clicked.connect(self.push_calculate)
        ui.push_save.clicked.connect(self.push_save)
        # self.ui.push_train.clicked.connect(self.push_train)
        # self.ui.push_load_excel.clicked.connect(self.push_load_excel) 
        # self.ui.push_preprocess.clicked.connect(self.push_preprocess)
        # self.ui.push_test_draw.clicked.connect(self.push_test_draw)
        # self.ui.push_predict.clicked.connect(self.push_predict)
        # self.ui.push_load_ini_model.clicked.connect(self.load_ini_model)

    def auto_resize(self):## 自适应屏幕大小
        # 获取窗口原来大小
    ...
    def push_save(self):
        ...
        # print(variables)
        cars_electricity=variables['cars_electricity']
        total_power_generation = variables['total_power_generation']
        total_power_used = variables['total_power_used']

        discharge_strategy = variables['discharge_strategy']
        cars_at_office_24h = variables['cars_at_office_24h']

        building_power_consumption = variables['building_power_consumption']
        position_24h = variables['position_24h']

        strategy_special = variables['strategy_special']
        voltage = variables['voltage']
        power_consumption_rate = variables['power_consumption_rate']
        ...
        pd.DataFrame(power_ratio,index=['总发电量','总用电量','消纳比例']).to_excel('output/Power_ratio.xlsx')

        # 保存每辆车每天的充电策略到excel
        electricity_n_days=pd.concat([pd.DataFrame(i).T for i in cars_electricity],axis=0)
        electricity_n_days.to_excel('output/soc_24h.xlsx')

        # 保存每辆车每天的充电策略到excel
        strategy_n_days=pd.concat([pd.DataFrame(i).T for i in discharge_strategy],axis=0)
        strategy_n_days.to_excel('output/strategy_24h.xlsx')

        # 保存每辆车每天的充电策略到excel
        discharge_strategy_n_days=pd.concat([pd.DataFrame(i).T for i in discharge_strategy],axis=0)
        discharge_strategy_n_days.to_excel('output/discharge_strategy_24h.xlsx')

        # 保存办公室每个时间段的车辆数
        cars_num_office=pd.concat([pd.DataFrame(i).T for i in cars_at_office_24h],axis=0)
        cars_num_office.to_excel('output/cars_num_office.xlsx')

        # 保存建筑整体功率数据
        pd.concat([pd.DataFrame(i) for i in building_power_consumption],axis=0).to_excel('output/building_power_consumption.xlsx')

        # 保存每个时间段的车辆位置
        pd.concat([pd.DataFrame(i).T for i in position_24h],axis=0).to_excel('output/position_24h.xlsx')

        # 保存特殊的策略
        pd.concat([pd.DataFrame(i).T for i in strategy_special],axis=0).to_excel('output/strategy_special.xlsx')

        # 保存电压变化情况
        pd.concat([pd.DataFrame(i).T for i in voltage],axis=0).to_excel('output/voltage.xlsx')

        # 保存功率消纳率
        pd.concat([pd.DataFrame(i).T for i in power_consumption_rate],axis=0).to_excel('output/power_consumption_rate.xlsx')
    
    def push_calculate(self):
        ui=self.ui
        globals_dict = {}
        locals_dict = {}
        variables = self.main() # 进行模拟
    ...
    def main(self):
        ...
        # %%
        # 定义时间步长和模拟天数
        step = float(self.ui.step.text())  # 时间步长（小时）
        # num_of_days = 3  # 模拟的天数
        # extra_days = 3  # 额外的天数，可能用于特殊处理或预热期
        num_of_days = int(self.ui.num_of_days.text())  # 模拟的天数
        extra_days = int(self.ui.extra_days.text())  # 额外的天数，可能用于特殊处理或预热期

        # 光伏装机功率
        installed_power = int(self.ui.installed_power.text())
        ...
        file = open("December_Unreal.csv", 'r') #Select the month
        ...
        # 读取2020年的实际光伏数据
        max_power_supply = np.zeros((366, int(24/step)))
        file = open("2020_Unreal.csv", 'r') #Select the month
        for line_index, line_str in enumerate(file):
            for i in range(int(24/step)):
                line_list = split_string_to_data(line_str)
