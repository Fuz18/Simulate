...
def charging_strategy(pre_location, pre_arriving, day, time):
    ...
    if max(co_charge) != 0:
        ...
        # 根据SOC级别更新停车场的车辆统计
        for i in range(number_of_cars):
            soc_level = min(int(cars[i, 3] * 10), 9)  # 确保索引不会超过9
            cars_at_office_24h[soc_level, int(time / step)] += 1
...
def pick_Markov_matrix(i, day, time, to_office, at_office):
    ...
    if time < to_office:
        ...
        if go_out_leisure_time[i] == 2:
            return Markov_matrix_list[5]
        elif go_out_leisure_time[i] == 0:
...
def generate_otherplace_distance(time, day, number_of_cars):
    ...
    if day % 7 == 1 or day % 7 == 0:  # 周末
        ...
        if time < 8:
            ...
            office_to_other = np.random.gamma(1.8, 2.86666, number_of_cars) * go_out_during_work_time
        elif time < 13:
            office_to_other = np.random.gamma(1.9, 1.579, number_of_cars)
        elif time < 18:
            office_to_other = np.random.gamma(1.8, 2.86666, number_of_cars) * go_out_during_work_time
...
# %%
# 初始化上班期间和休闲时间的外出行为模式
go_out_during_work_time = go_out_during_work_time(percentage_to_go_out_during_working, number_of_cars)
go_out_leisure_time = go_out_leisure_time(percentage_to_go_out_leisure_time, number_of_cars)

# 打印验证数据
print(go_out_during_work_time)
print(go_out_leisure_time)
